...
function fetchAndRenderData(URL, cacheKey_liga3, renderFunction) {
    fetch(URL)
        .then(res => res.text())
        .then(rep => {
            ...
            localStorage.setItem(cacheKey_liga3, JSON.stringify(cacheData));
...
function isCacheValid(cacheKey_liga3) {
    let cached = JSON.parse(localStorage.getItem(cacheKey_liga3));
...
function loadMatchData(day) {
    ...
    let cacheKey_liga3 = `spielplan_day_${day}`;

    if (isCacheValid(cacheKey_liga3)) {
        // Lade Daten aus dem Cache
        let cachedData = JSON.parse(localStorage.getItem(cacheKey_liga3)).data;
        renderMatchTable(cachedData);
    } else {
        // Lade Daten von Google Sheets und rendere die Tabelle
        fetchAndRenderData(URL, cacheKey_liga3, renderMatchTable);
    }
}
